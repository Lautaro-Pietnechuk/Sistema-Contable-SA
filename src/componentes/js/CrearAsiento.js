  ...
  const CrearAsiento = () => {
    ...
    const [transacciones, setTransacciones] = useState([{ cuenta: "", tipo: "debe", monto: "", bloqueada: false }]);
    ...
    const handleAddTransaccion = () => {
      setTransacciones([...transacciones, { cuenta: "", tipo: "debe", monto: "", bloqueada: false }]);
    };
    ...
    const handleToggleBloqueo = (index) => {
      const nuevasTransacciones = [...transacciones];
      nuevasTransacciones[index].bloqueada = !nuevasTransacciones[index].bloqueada;
      setTransacciones(nuevasTransacciones);
    };
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          setTransacciones((prev) => {
            const transaccionesBloqueadas = prev
            ...
            if (transaccionesBloqueadas.length > 0) {
              return transaccionesBloqueadas;
            }

            return [{ cuenta: "", tipo: "debe", monto: "", bloqueada: false }];
          });
          setMensajeError("");
        }
      } catch (error) {
              ...
              {transacciones.map((transaccion, index) => (
                <tr key={index}>
                  ...
                  <td className="acciones-celda">
                    <button
                      className="crear-asiento-boton eliminar accion-boton"
                      type="button"
                    ...
                    <button
                      className="crear-asiento-boton accion-boton bloqueo-boton"
                      type="button"
                      onClick={() => handleToggleBloqueo(index)}
                      title={transaccion.bloqueada ? "Desbloquear transacción" : "Bloquear transacción"}
                      aria-label={transaccion.bloqueada ? "Desbloquear transacción" : "Bloquear transacción"}
                    >
